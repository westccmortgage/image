...
import { fieldLabel, fieldQuestion, fieldOptionLabel } from './fieldsI18n';
...
import type { FieldKey, Language, ScenarioProfile } from './types';
...
// at most 5 known facts and at most 4 critical missing items. Fully localized.
...
function displayValue(lang: Language, key: FieldKey, p: ScenarioProfile): string {
  const v = p[key];
  if (v == null || v === '') return '';
...
export function buildCompactProfile(p: ScenarioProfile, lang: Language = 'en'): CompactProfile {
  ...
  if (d.loanAmount != null && facts.length < MAX_FACTS) {
    facts.push({ key: 'purchasePrice', label: t(lang, 'loanAmountLabel'), value: formatMoney(d.loanAmount) });
  }
